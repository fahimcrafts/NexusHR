Feature: Leave Module Navigation
  ...
  Scenario: Admin assigns leave to the employee
    Given I am on the login page
    When I login with valid admin credentials
    And I create a new employee for the leave workflow
    ...
    And I select Assign Leave employee for the leave workflow
    ...
    And I navigate to Leave module
    And I select in Leave List employee for the leave workflow
    And I select leave status "Taken"
    And I click in Leave List search button
    Then The leave record should show employee for the leave workflow, leave type "CAN - FMLA", from "2026-08-24", to "2026-08-25", and status "Taken"